...
<div class="content">
    ...
    <div class="content_right" id="content_right">
   	...
   	<form action="SearchNurse" method="post">
   		<input type="text" name="search"  placeholder="请输入护士编号" required><input type="submit"  value="搜索">&nbsp; <input type="button" onclick="toaddNurse()" value="添加" id="n1">
   	</form>
   	<table  border="1">
    <tr>
      <th>编号</th>
     ...
     <c:if test="${!empty Nurselist }">
				<c:forEach items="${Nurselist}" var="nurse">
					<tr>
						...
						<td>${nurse.hNo }</td>
						<td>在职</td>
						<td><a href = "ModifyNurse?nNo=${nurse.nNo }">修改</a></td>
      					<td><a href="">离职办理</a></td>
					</tr>				
				</c:forEach>
			</c:if>
