...
function fun(){
	...
	function toaddLogistic(){
		window.location.href = 'addLogistic';
	} 
...
<div class="content">
    ...
    <div class="content_right" id="content_right">
    	<h4 align="center">后勤人员信息区</h4>&nbsp;&nbsp;&nbsp;
   	<form action="" method="post">
   		<input type="text" name="search"  placeholder="请输入后勤人员编号" required><input type="submit"  value="搜索">&nbsp <input type="button" onclick="toaddLogistic()" value="添加" id="l1">
   	</form>
   	<table  border="1">
    <tr>
      <th>编号</th>
    ...
    <c:if test="${!empty Logisticlist }">
				<c:forEach items="${Logisticlist }" var="logistic">
					<tr>
						<td>${logistic.lNo}</td>
						<td>${logistic.lName }</td>
						<td>${logistic.aNo}</td>
						<td>${logistic.lSex}</td>
						<td>${logistic.lTel }</td>
						<td>${logistic.lAge }</td>
						<td>${logistic.lState }</td>
						<td><a href="ModifyLogistic?lNo= ${logistic.lNo }">修改</td>
      					<td><a href="">离职办理</a></td>
					</tr>				
				</c:forEach>
			</c:if>
